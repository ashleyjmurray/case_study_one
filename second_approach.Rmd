...
second_approach <- function() {
  ...
  beta_0 ~ dnorm(0.0, 1.0E-2) # Prior on beta_0 is normal with low precision
  beta_1 ~ dnorm(0.0, 1.0E-2) # Prior on beta_1 is normal with low precision
  beta_2 ~ dnorm(0.0, 1.0E-2) # Prior on beta_2 is normal with low precision
  beta_3 ~ dnorm(0.0, 1.0E-2) # Prior on beta_3 is normal with low precision
  beta_4 ~ dnorm(0.0, 1.0E-2) # Prior on beta_4 is normal with low precision
  beta_5 ~ dnorm(0.0, 1.0E-2) # Prior on beta_5 is normal with low precision
  beta_6 ~ dnorm(0.0, 1.0E-2) # Prior on beta_6 is normal with low precision
  beta_7 ~ dnorm(0.0, 1.0E-2) # Prior on beta_7 is normal with low precision
  beta_8 ~ dnorm(0.0, 1.0E-2) # Prior on beta_8 is normal with low precision
  beta_9 ~ dnorm(0.0, 1.0E-2) # Prior on beta_9 is normal with low precision
  r ~ dexp(1) # Prior on r
  ...
  survival_Obama ~ dweib(r, mu_Obama) %_% T(present_length, upper_length) 
  age_Obama_predictive <- survival_Obama 
  
  beta_Daila <- beta_0 + beta_4*year_Daila + beta_8*year_Daila
  mu_Daila <- exp(beta_Daila)
  survival_Daila ~ dweib(r, mu_Daila) %_% T(present_length, upper_length) 
  age_Daila_predictive <- survival_Daila
  
  beta_Naruhito <- beta_0 + beta_5*year_Naruhito + beta_9*year_Naruhito
  mu_Naruhito <- exp(beta_Naruhito)
  survival_Naruhito ~ dweib(r, mu_Naruhito) %_% T(present_length, upper_length) 
...
present_length <- leaders_data$Age.Event[leaders_data$Name =="Barack Obama"]

birth_year_Daila <- leaders_data$Birth.Year[leaders_data$Name == "14th Dalai Lama (Tenzin Gyatso)"]
year_Daila <- birth_year_Daila - mean(leaders_data$Birth.Year) # center on leaders or leaders_nopred?
present_length <- leaders_data$Age.Event[leaders_data$Name =="14th Dalai Lama (Tenzin Gyatso)"]
...
present_length <- leaders_data$Age.Event[leaders_data$Name =="Naruhito (Emperor Reiwa)"]
...
Bayesian_Popes_second_approach <- jags(data = data_Popes_second_approach,  
                                      parameters.to.save = c("beta_0",
                                                             ...
                                                             "age_Obama_predictive","age_Daila_predictive","age_Naruhito_predictive"), 
                                      n.iter = 50000, 
                                      n.chains = 3,
...
# btw the predictions for the four leaders are somewhat sus (like Obama isn't going to die at 60, and 60.435 shows up a lot in our results)…so maybe there's a mistake or model diagnostics will tell us more about our model's predictive ability --Cathy
