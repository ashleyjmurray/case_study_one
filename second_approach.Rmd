...
build_model <- function() {
  ...
  beta_Benedict <- beta_0 + beta_1*year_Benedict
  ...
  beta_Carter <- beta_0 + (beta_1+beta_6)*year_Carter + beta_2
  mu_Carter <- exp(beta_Carter)
  survival_Carter ~ dweib(r, mu_Carter) %_% T(present_length_Carter, upper_length)
  age_Carter_predictive <- survival_Cater
  
  beta_Clinton <- beta_0 + (beta_1+beta_6)*year_Clinton + beta_2
  ...
  beta_Bush <- beta_0 + (beta_1+beta_6)*year_Bush + beta_2
  mu_Bush <- exp(beta_Bush)
  survival_Bush ~ dweib(r, mu_Bush) %_% T(present_length_Bush, upper_length)
  age_Bush_predictive <- survival_Bush
  
  beta_Trump <- beta_0 + (beta_1+beta_6)*year_Trump + beta_2
  mu_Trump <- exp(beta_Trump)
  survival_Trump ~ dweib(r, mu_Trumo) %_% T(present_length_Trump, upper_length)
  age_Trump_predictive <- survival_Trump
  
  beta_Akihito <- beta_0 + (beta_1+beta_9)*year_Akihito + beta_5
  mu_Akihito <- exp(beta_Akihito)
...
ggplot(four_pred, aes(x=four_pred[,i])) + geom_histogram(aes(y=..density..), 
  ...
  annotate("text", x = minval+0.15*(medval-minval), y = 0.075, label = paste("Current age", minval, "years", sep = "\n"), size = 3)+
  annotate("text", x = (medval+minval)/2, y = 0.055, label = "50%", size = 3, color = "blue")+
  annotate("text", x = medval+0.15*(val95-medval), y = 0.075, label = paste("Median", medval,"years", sep = "\n"), size = 3)+
  annotate("text", x = (medval+val95)/2, y = 0.055, label = "45%", size = 3, color = "blue")+
  annotate("text", x = val95+2.2, y = 0.075, label = paste("95%", "quantile", val95, "years", sep = "\n"), size = 3)+
  annotate("text", x = val95+3, y = 0.055, label = "5%", size = 3, color = "blue")
}
...
ggplot(francis_dalai, aes(x=plifespans)) + 
  ...
  labs(caption = "The posterior predictive probability density function of the lifespan for hypothetical leaders with the same attributes as Pope Francis (left) and the 14th Dalai Lama (right). The black vertical line marks current age, the blue marks posterior median, and the red marks the posterior 95% quantile.") +
  ylim(c(0, 0.1)) + 
  geom_text(data = plot_txt, mapping = aes(x = 88.5, y = 0.09, label = label), size = 2.8)
```
...
ggplot(obama_naru, aes(x=plifespans)) + 
  ...
  geom_hline(yintercept = 0, colour = "black") +
  xlab("Lifespan (yrs)") + ylab("Predictive Probability density") +
  labs(caption = paste("Fig. 3 The posterior predictive probability density function of the lifespan for hypothetical leaders with the same attributes", "as Emperor Naruhito (left) and Obama (right). The black vertical line marks current age, the blue marks posterior", "median, and the red marks the posterior 95% quantile.", sep = "\n")) +
