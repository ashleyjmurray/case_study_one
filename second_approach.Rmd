...
mean(four_pred$age_Dalai_predictive>four_pred$age_Francis_predictive)
mean(four_pred$age_Obama_predictive>four_pred$age_Naruhito_predictive)
```
...
#     (age_Francis_predictive <= medval) ~ "a",
#     (age_Francis_predictive > val95) ~ "c",
#     TRUE ~ "b"))

ggplot(four_pred, aes(x=four_pred[,1])) + geom_histogram(aes(y=..density..), 
                                                   binwidth = 0.8,
                                                   boundary = minval) +
  ...
  ggtitle("Predictive Posterior Distribution for Lifespan of Pope Francis") +
  ylim(c(0, 0.07))+
  annotate("text", x = minval+2.5, y = 0.06, label = paste("Current xge", minval, "years", sep = "\n"), size = 3)+
  annotate("text", x = (medval+minval)/2, y = 0.05, label = "50%", size = 3)+
  annotate("text", x = medval+1.5, y = 0.06, label = paste("Median", medval,"years", sep = "\n"), size = 3)+
  annotate("text", x = (medval+val95)/2, y = 0.05, label = "45%", size = 3)+
  annotate("text", x = val95+2.5, y = 0.06, label = paste("95% quantile", val95, "years", sep = "\n"), size = 3)+
...
ggplot(obama_naru, aes(x=plifespans)) + 
  ...
  xlab("Lifespan (years)") +
  ylab("Predictive Probability density") +
  ggtitle(paste("Predictive Posterior Distribution for Lifespans of Obama and Naruhito", name)) +
  ylim(c(0, 0.05)) #+
  annotate("text", x = minval+0.15*(medval-minval), y = 0.06, label = paste("Current age", minval, "years", sep = "\n"), size = 3)+
  # annotate("text", x = (medval+minval)/2, y = 0.05, label = "50%", size = 3)+
  # annotate("text", x = medval+0.15*(val95-medval), y = 0.06, label = paste("Median", medval,"years", sep = "\n"), size = 3)+
