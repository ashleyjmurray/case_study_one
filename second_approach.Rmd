...
## Plots of Posterior Predicitive distribution of Lifespans 
...
ggplot(francis_dalai, aes(x=plifespans)) + 
  ...
  ggtitle(paste("Posterior Predicted Lifespans (Pope Francis and 14th Dalai Lama)")) +
  ylim(c(0, 0.05)) + 
...
ggplot(obama_naru, aes(x=plifespans)) + 
  ...
  geom_hline(yintercept = 0, colour = "black") +
  xlab("Lifespan (yrs)") + ylab("Predictive Probability density") +
  ggtitle(paste("Posterior Predicted Lifespans (President Obama and Emperor Naruhito)")) +
  ylim(c(0, 0.05)) + 
  geom_text(data = plot_txt, mapping = aes(x = 80, y = 0.04, label = label), size = 3)
```

```{r tidy-model-table}
output <- data.frame(Bayesian_Popes_second_approach$BUGSoutput$summary)
output <- output %>%
  mutate("Variable" = Bayesian_Popes_second_approach$BUGSoutput$root.short,
         "Mean" = output$mean,
