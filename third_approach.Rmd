...
```{r, fig.height = 4, fig.width=10, echo=F,message=FALSE}
p1 <- ggplot(data=leaders_data, aes(x=Birth.Year, y = Age.Event, color = Type)) +
  geom_point() + geom_smooth(method="lm", se = F) + 
  labs(title = "Figure 1: Birth Year vs. Lifespan by Leadership Types", x = "Birth Year", y = "Age of Death/Censorship") + 
...
p2 <- ggplot(data = leaders_data) +
  geom_boxplot(aes(x = Type, y = Age.Event)) + 
  labs(title = "Figure 2: Distribution of Leader Survival Time", x = "Type of Leader", y = "Age of Death/Censorship") +
  theme(plot.title = element_text(hjust = 0.5, size = 10),
        axis.title = element_text(size = 8))
...
$$
+ \sum_{j = 4, \cdots, 6,\ k = 7}\beta_{j,k} I(Birth \ Century_i = j)*I(Leadership_i = k) 
$$

$$
+ \sum_{j = 1, \cdots, 6,\ k = 8, \cdots, 10}\beta_{j,k} I(Birth \ Century_i = j)*I(Leadership_i = k)
$$

where the indicator functions $Birth \ Century_i = 1, \cdots, 6$ corresponds to a leader $i$ born in the 15th, 16th, 17th, 18th, 19th and 20th century respectively. The 14th century is the baseline for comparison. $Leadership_i = 7, \cdots, 10$ corresponds to a leader $i$ being a U.S. President, a Chinese Emperor, a Dalai Lamai and a Japanese Emperor respectively. $Leadership_i$ uses Pope as the baseline for comparison. $\beta_{j,k}$ is the parameter for the interaction effect between birth year century $j$ and leadership type $k$. 
...
As Stander et al (2018) pointed out, following this Weibull distribution, $\log(T)$ is identical in distribution to $\frac{1}{r}(\mu)+\frac{1}{r}(\epsilon)$, or
...
The interpretation of coefficients depends on the interaction terms (i.e. both birth year century and the types of leadership). For example, while keeping all else constant, if a Pope was born in the 20th century, he is expected to live longer by a multiplicative factor of $exp(\alpha_6)$ compared to a Pope born in the 14th century, or his lifespan is expected to increase by a percentage of $100*exp(\alpha_6-1)$. If a U.S. President was born in the 18th century, he is expected to live $exp(\alpha_0+\alpha_4+\alpha_7+\alpha_{4,7})$ years; if a Chinese Emperor and a Dalai Lama were both born in the 19th century, the Chinese emperor is expected to live longer by a multiplicative factor of $exp(\alpha_8 - \alpha_9 + \alpha_{5,8}-\alpha_{5,9})$ and so on. 

# V. Results

## V a. Output and Interpretation of the Model

Our analysis below will be based on the multiplicative factors, $\alpha$'s. Note that since $\alpha = -\beta/r$ is a monotone transformation, if $\beta$ is statistically significant (i.e. 95% credible interval doesn't contain 0), it should be the same for the corresponding $\alpha$. A compete table of the model output can be found in Appendix A.4.
...
kable(JapanEmp_diff_time, caption = "Change in Lifespan by Birth Century for Japan Emperors")
```
...
kable(Dalai_diff_time, caption = "Change in Lifespan by Birth Century for Dalai Lamas")
```
...
kable(diff_leaders_18, caption = "Change in Lifespan by Leader Type for Leaders Born in the 18th Century")
```
...
kable(diff_leaders_20,1, caption = "Change in Lifespan by Leader Type for Leaders Born in 19th Century")
```

From Table 2 and Table 3 we can see the effect of birth year century depends on leader types. Popes born in the 18th and 19th centuries are expected to live longer than other leaders born in the same century. If born in the 18th century, a U.S. president and a Chinese emperor are expected to enjoy comparable lifespans as a Pope. However, if born in the 19th century instead, the expected lifespans of a U.S. president and a Chinese emperor are significantly shorter than that of a Pope (`r (-1)*diff_leaders_20[2,3]`% and `r (-1)*diff_leaders_20[3,3]`% shorter respectively). More examples of the interpretation of coefficients are in Appendix A.3. 
...
kable(coef, digits = 1, caption = "Predicted Lifespans for all Living Leaders")
```
...
p1 = ggplot(DalaivFrancis) + 
  ...
  labs(caption = paste("Fig. 3a This density plot shows the posterior distribution of the", "probability that the 14th Dalai Lama outlives Pope Francis.", sep = "\n"), size = 4) +
...
```{r resid-v-year, fig.height=3, fig.width=10, echo = FALSE}

p1 <- ggplot(data = leaders_nopred, aes(x = Ts, y = resid/sd)) + geom_point() + 
  labs(x = "Predicted Lifespan")

p2 <- ggplot(data = leaders_nopred, aes(x = Birth.Year, y = resid/sd)) + geom_point() +
  labs(x = "Birth Year")

grid.arrange(p1, p2, left = "Standardized Residual", ncol=2,
             top ="Figure 4: Residual Plots")
```

The standardized residuals plotted against birth year are randomly scattered, showing that our model doesn't systematically over- or under-predict lifespan based on birth year. The standardized residuals vs. predicted lifespan are all still fairly randomly scattered, however there might be some trending for predicted lifespans greater than 75. Some of this trending might be because we do not have enough predictors in our model. It is also worth pointing out that we have a few standardized residuals above 2 and below -2, meaning we might have some outliers. However, none of them cross the 3 or -3 threshold, so none of them are extreme outliers.
...
#density distribution of the ten simulations + the actual non-censored data
...
ggplot(aes(x = Age, group = Simulation_Number_or_the_Actual_Data, 
  ...
  ggtitle("Figure 5: Posterior Predictive Checks Density Plot") +
  theme(plot.title = element_text(size = 10, hjust = 0.5),
        axis.title = element_text(size = 8),
        legend.title = element_text(size = 7))
```

Figure 5 showcases the curvatures of the ten different simulations done in the posterior predictive checks and how they relate to the actual age of the non-censored individuals. There appears to be a peak within the non-censored individuals at around 70, but most of the simulations are seeing a peak at near 100. For the posterior predictive checks, it appears that our model returns values that are more concentrated around the upper truncation limit, which is why the dataframe's Inf values are encoded as 100 (the upper truncation limit defined in our function). This follows what the standardized residuals found, where some of the values from the model are left-skewed, and the posterior predictive checks is showing that a lot of the leaders are being predicted to having a larger lifespan.
...
```{r, sensitivity-analysis, warnings = F, fig.width=10, fig.height = 4, echo = F}
parameter = "beta_6"
...
sa_df = data.frame(vals = rbind(sa1, sa2, sa3, sa4, sa5, sa6),
                   type = c(rep("r~Exp(0.001), B~N(0,1)", nrow(sa1)),
                            ...
                            rep("r~Ga(10,100), B~N(0,1)", nrow(sa3)),
                            rep("r~Exp(0.001), B~N(0,10000)", nrow(sa4)),
                            rep("r~Ga(10,10), B~N(0,10000)", nrow(sa5)),
                            rep("r~Ga(10,100), B~N(0,10000)", nrow(sa6))))

neworder <- c("r~Exp(0.001), B~N(0,1)", "r~Ga(10,10), B~N(0,1)", "r~Ga(10,100), B~N(0,1)",
              "r~Exp(0.001), B~N(0,10000)", "r~Ga(10,10), B~N(0,10000)", "r~Ga(10,100), B~N(0,10000)")
...
ggplot(sa_df_plot) + geom_density(aes(x = beta_6)) + 
  ...
  labs(x = expression(beta[6], y = "Density"), 
       title = "Figure 6: Sensitivity Analysis for Different Priors",
       caption = paste("The blue line indicates the estimate of the coefficient from our model. It appears that the analysis is generally not sensitive to different priors.")) +
  theme(plot.caption = element_text(hjust = 0.5, vjust = -0.5, size = 11),
        plot.title = element_text(hjust = 0.5))
``` 

We tested $N(0,1)$ and $N(0,10000)$ priors for the $\beta$'s and $Exp(0.001)$ (Stander et al prior), $Gamma(10,10)$ (mean = 1), and $Gamma(10,100)$ (mean = 0.1) priors for $r$. Distribution of $\beta_6$ was approximated using those different priors and plotted above. The blue line, which is the estimate for $\beta_6$ from our model, crosses (or is close to) the mode of all but one of the distributions, so our analysis is mostly robust against changes in prior. However, it is somewhat sensitive to the prior of $r \sim Gamma(10,100)$, which makes sense since that prior implies a decreasing hazard while we assumed increasing hazard. Code to conduct sensitivity analysis for the remaining betas can be found in Appendix A.4.

# VII. Cross Validation

We sought to perform external cross-validation on our model to assess how useful our model would be to predict lifespans of other leaders. The data that we used for the cross-validation was from all of England's monarchs born in or after the 1300s, accessed from Wikipedia. This dataset only included one person that was still alive (Queen Elizabeth II). We assigned leader type `JapanEmp` to all British monarchs since we felt this might be the closest match, given China doesn't have any emperors after 1911. 
...
```{r resid-v-predictorcv, include = FALSE}
p1 = ggplot(data = monarchs, aes(x = Ts, y = resid/sd)) + geom_point() + 
  labs(x = "Predicted Lifespan")  +
  theme(axis.title.y = element_blank()) 
```

```{r resid-v-yearcv, fig.height=3, fig.width=10, echo = FALSE}
p2 = ggplot(data = monarchs, aes(x = V2, y = resid/sd)) + geom_point() +
  labs(x = "Birth Year") +
  theme(axis.title.y = element_blank()) 

grid.arrange(p1, p2, left = "Standardized Residual", ncol=2,
             top ="Figure 7: Residual Plots for British Monarchs")
```

As shown in the standardized residuals plots above, the our model didn't do a good job predicting how long British monarchs could live based on their birth year - standardized residuals are very large. In the plot showing Predicted Lifespan and the standardized residuals, it appears to have a downward trend. As the birth year of the monarchs increases, there is an upward trend. It seems that our model is underpredicting for leaders in the later centuries. 
...
List of English monarchs. (2020, September 08). Retrieved September 08, 2020, from https://en.wikipedia.org/wiki/List_of_English_monarchs
...
res_lag1 = function(i) { # plot for all parameters
  plot(y=res[,i], x= lag1_df[[i]], 
       ...
       main = paste("Lag-1 Scatterplot of", colnames(res)[i]))
}
```
...
plot(y=res[,2], x= lag1_df[[2]], 
       ...
       main = paste("Lag-1 Scatterplot of", colnames(res)[2]))
```
...
Prior to arriving at our current model, we explored to other models for $\log(\mu_i)$:

1. Birth year as a continuous variable.
...
where $Leadership_i$ = $Pope$ is the baseline for comparison. 

2. Birth year as a continuous variable and quadratic polynomial model.

$$\log(\mu_i) = \beta_0+\beta_1 (Year \ of \ Birth_i)+\beta_2 (Year \ of \ Birth_i)^2 + \beta_3(Leadership_i = UsPres) \\$$
$$+ \beta_4 (Leadership_i = ChinaEmp) + \beta_5 (Leadership_i = DalaiLama) +\beta_6 (Leadership_i = JapanEmp) \\$$
$$+ \beta_7 (Year \ of \ Birth_i * (Leadership_i = UsPres)) + \beta_8 (Year \ of \ Birth_i * (Leadership_i = ChinaEmp)) \\$$
$$+ \beta_9 (Year \ of \ Birth_i * (Leadership_i = DalaiLama)) + \beta_9 (Year \ of \ Birth_i * (Leadership_i = JapanEmp))\\$$
$$+ \beta_{10} ((Year \ of \ Birth_i)^2 * (Leadership_i = UsPres)) + + \beta_{11} ((Year \ of \ Birth_i)^2 * (Leadership_i = ChinaEmp)) \\$$
$$+ \beta_{12} ((Year \ of \ Birth_i)^2 * (Leadership_i = DalaiLama)) + + \beta_{13} ((Year \ of \ Birth_i)^2 * (Leadership_i = JapanEmp)) \\$$
where $Leadership_i$ = $Pope$ is the baseline for comparison. 

## A.3 Model Output And More Interpretation Examples
```{r echo=F}
kable(coef_alpha, digits = 2)
```
...
The percentage increase in lifespan becomes generally more negative, which means later born Dalai Lamas are expected to live shorter, confirming the negative correlation between lifespan and birth year exihibited in Figure 1 of section II.b EDA. The Dalai Lama born in the 20th century, however, is expected to live abnormally longer than the 1st Dalai Lama born in the 14th century, due to the extremely large point estimate of $\alpha_{6,9}$ that corresponds to the interaction between born in the 20th century and being a Dalai Lama. As explained earlier, this might be because there is only one Dalai Lama born in the 20th century and he is censored. 
...
## A.4 Code for Sensitivity Analysis
...
## A.5 Weibull Distributions with Differing r Values
...
## A.6 Histograms of Posterior Predictive Distribution for Lifespan of the 4 Selected Leaders
...
cap = paste("The posterior predictive probability density function of the lifespan for hypothetical leaders with the same attributes as Obama (right) and Emperor", "Naruhito (left). The black vertical line marks current age, the blue marks posterior median, and the red marks the posterior 95% quantile.", sep = "\n")
