...
Since the baseline for comparison is a Pope born in the 14th century, it's meaningful to interpret the intercept in this case. The intercept shows that we expect a Pope born in the 14th century to live exp($\alpha_0$) = exp(`r coef_alpha[1,3]`) = `r exp(coef_alpha[1,3])` years. 
...
```{r, echo=F}
leader_vec <- c("Born in 15th Century", "Born in 16th Century", "Born in 17th Century", "Born in 18th Century", "Born in 19th Century", "Born in 20th Century")
alpha_vals<- c(coef_alpha[2,3] + coef_alpha[14,3], coef_alpha[3,3]+coef_alpha[17,3], coef_alpha[4,3]+coef_alpha[20,3], coef_alpha[5,3]+coef_alpha[24,3], coef_alpha[6,3]+coef_alpha[28,3],coef_alpha[7,3]+coef_alpha[32,3])
multiplicative_factor <- round(exp(alpha_vals),3)
...
JapanEmp_diff_time <- data.frame(leader_vec) %>%
  mutate("Japanese Emperor" = leader_vec,
         ...
         "Percentage Increase / Decrease in Lifespan" = perc_increase) %>%
  select("Japanese Emperor","Multiplicative Factor","Percentage Increase / Decrease in Lifespan")
kable(JapanEmp_diff_time)
```
Surprisingly, the model suggests that longevity doesn't always increase with calendar time. The biggest increase in lifespan is in the 20th century, where the expected lifespan of a Japanese emperor increases by `r JapanEmp_diff_time[6,3]`% compared to a Japanese emperor born 6 centuries ago. This might make sense, given the last century saw the biggest technological breakthroughs and improvement in healthcare.   
...
- Leaders born in the 18th century
   + Pope: $\log(T_{15th,Pope}) = \alpha_0+\alpha_4+\frac{1}{r}\log(\epsilon)$
   + U.S. President: $\log(T_{15th,UsPres}) = \alpha_0+\alpha_4+\alpha_7+\alpha_{4,7}+\frac{1}{r}\log(\epsilon)$
   + Chinese Emperor: $\log(T_{15th,ChinaEmp}) = \alpha_0+\alpha_4+\alpha_8+\alpha_{4,8}+\frac{1}{r}\log(\epsilon)$
   + Dalai Lama: $\log(T_{15th,Dalai}) = \alpha_0+\alpha_4+\alpha_9+\alpha_{4,9}+\frac{1}{r}\log(\epsilon)$
   + Japanese Emperor: $\log(T_{15th,JapanEmp}) = \alpha_0+\alpha_4+\alpha_10+\alpha_{4,10}+\frac{1}{r}\log(\epsilon)$
...
- Leaders born in the 20th century
   ...
   + Japanese Emperor: $\log(T_{20th,JapanEmp}) = \alpha_0+\alpha_6+\alpha_10+\alpha_{6,10}+\frac{1}{r}\log(\epsilon)$
...
kable(coef, digits = 3)
```
The table above shows the estimate and 95% credible interval (the bounds are the 2.5% quantile and 97.5% quantile) for all 10 living leaders in the dataset.
