...
x_1 <- (leaders_nopred$Birth.Year - mean(leaders_nopred$Birth.Year))/100
x_1_2 <- (x_1)^2
...
x_6_2 <- x_1_2*as.numeric(as.character(x_2))
x_7 <- x_1*as.numeric(as.character(x_3))
x_7_2 <- x_1_2*as.numeric(as.character(x_3))

x_8 <- x_1*as.numeric(as.character(x_4))
x_8_2 <- x_1_2*as.numeric(as.character(x_4))

x_9 <- x_1*as.numeric(as.character(x_5))
x_9_2 <- x_1_2*as.numeric(as.character(x_5))
```
...
build_model <- function() {
  for(i in 1:n_censored) {
    ...
    beta_censored[i] <- beta_0 + beta_1*x_1_censored[i] + beta_1_2*x_1_2_censored[i] +
      beta_2*x_2_censored[i] + beta_3*x_3_censored[i] + beta_4*x_4_censored[i] + beta_5*x_5_censored[i] + beta_6*x_6_censored[i] + beta_6_2*x_6_2_censored[i]+ beta_7*x_7_censored[i] +beta_7_2*x_7_2_censored[i] + beta_8*x_8_censored[i] + beta_8_2*x_8_2_censored[i]+ beta_9*x_9_censored[i] +beta_9_2*x_9_2_censored[i]
  ...
  for(j in 1:n_non_censored) { #total rows - 6 ** 6 are censored in leaders_nopred
    ...
    mu[j] <- exp(beta[j])
    beta[j] <- beta_0 + beta_1*x_1_non_censored[j] + beta_1_2*x_1_2_non_censored[j] +
      beta_2*x_2_non_censored[j] + beta_3*x_3_non_censored[j] + beta_4*x_4_non_censored[j] + beta_5*x_5_non_censored[j] + beta_6*x_6_non_censored[j] + beta_6_2*x_6_2_non_censored[j]+ beta_7*x_7_non_censored[j] +beta_7_2*x_7_2_non_censored[j] + beta_8*x_8_non_censored[j] + beta_8_2*x_8_2_non_censored[j]+ beta_9*x_9_non_censored[j] +beta_9_2*x_9_2_non_censored[j]
  }
  ...
  beta_1_2 ~ dnorm(0.0, 1.0E-3) # Prior on beta_1 is normal with low precision
  ...
  beta_6_2 ~ dnorm(0.0, 1.0E-3) # Prior on beta_1 is normal with low precision
  beta_7 ~ dnorm(0.0, 1.0E-3) # Prior on beta_7 is normal with low precision
  beta_7_2 ~ dnorm(0.0, 1.0E-3) # Prior on beta_1 is normal with low precision
  beta_8 ~ dnorm(0.0, 1.0E-3) # Prior on beta_8 is normal with low precision
  beta_8_2 ~ dnorm(0.0, 1.0E-3) # Prior on beta_1 is normal with low precision
  beta_9 ~ dnorm(0.0, 1.0E-3) # Prior on beta_9 is normal with low precision
  beta_9_2 ~ dnorm(0.0, 1.0E-3) # Prior on beta_1 is normal with low precision
  ...
  beta_Obama <- beta_0 + (beta_1+beta_1_2+beta_6+beta_6_2)*year_Obama + beta_2
...
x_1_2_censored <- x_1_2[censored_dex]
...
x_6_2_censored <- x_6_2[censored_dex]
x_7_censored <- x_7[censored_dex]
x_7_2_censored <- x_7_2[censored_dex]
x_8_censored <- x_8[censored_dex]
x_8_2_censored <- x_8_2[censored_dex]
x_9_censored <- x_9[censored_dex]
x_9_2_censored <- x_9_2[censored_dex]
...
x_1_2_non_censored <- x_1_2[-censored_dex]
...
x_6_2_non_censored <- x_6_2[-censored_dex]
x_7_non_censored <- x_7[-censored_dex]
x_7_2_non_censored <- x_7_2[-censored_dex]
x_8_non_censored <- x_8[-censored_dex]
x_8_2_non_censored <- x_8_2[-censored_dex]
x_9_non_censored <- x_9[-censored_dex]
x_9_2_non_censored <- x_9_2[-censored_dex]
...
p1=ggplot(DalaivFrancis, aes(x=diff)) + 
  ...
  annotate("text", x = 1, y = 0.04, label = paste("mean =", round(meandf,2)), size=3.5, color = "blue") + 
...
p2=ggplot(ObamavNaruhito, aes(x=diff)) + 
  geom_histogram(aes(y = ..density..), binwidth = 1) +
   geom_vline(xintercept = c(meandf),color="blue")+
      ...
      xlab("Difference in Lifespan") +
      theme(axis.title.y = element_blank()) + 
  annotate("text", x = 3, y = 0.02, label = paste("mean =", round(meandf,2)), size=3.5, color = "blue") + 
...
data.mat = as.matrix(cbind(x_1, x_1_2, x_2, x_3, x_4, x_5, x_6, x_6_2, x_7, x_7_2, x_8, x_8_2, x_9, x_9_2))
...
r = pred_res$r[1]
betas = pred_res[,c("beta_0", "beta_1", "beta_1_2", "beta_2",  "beta_3", "beta_4", "beta_5",
                     "beta_6",  "beta_6_2", "beta_7", "beta_7_2", "beta_8", "beta_8_2", "beta_9", "beta_9_2")][1,]
